...
def compare_ma1e(img_true, img_test):
  
  img_true = torch.from_numpy(img_true.astype(np.float32)/255)
  img_test = torch.from_numpy(img_test.astype(np.float32)/255)
  loss = l1_loss(img_test, img_true)
  mask = np.zeros_like(img_test)[:, :, 0]
  mask[64:192,64:192] = 255
  mask = torch.from_numpy(mask.astype(np.float32) / 255.0)
  print(loss/torch.mean(mask))
  sys.exit()
